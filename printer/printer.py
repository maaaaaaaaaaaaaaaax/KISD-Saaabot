...
class Printer:
    ...
    def text(self, content: str, style: Optional[Union[TextStyle, str]] = None) -> 'Printer':
        """
    ...
    def print_text(self, content: str, style: Optional[Union[TextStyle, str]] = None) -> 'Printer':
        """
    ...
    def print_image(
        ...
        try:
            ...
            if self.config.auto_resize and width > 100:
                print(f"Warning: Image processing failed, retrying with reduced size: {e}")
                return self.print_image(source, width=width // 2, rotation=rotation, alignment=alignment, high_density=high_density)
            else:
                raise
    ...
    def print_separator(self, char: str = '-', rows: int = 2, width: Optional[int] = None) -> 'Printer':
        """
        ...
        """
        if width is None:
            width = self.config.get_chars_per_line(self._current_font, self._current_width_multiplier)
    ...
    def barcode(
        ...
        Returns:
            Self for chaining
        """
        self.device.barcode(code, bc_type, height=height, width=width, pos=pos, font=font)
        return self
